...
server <- function(input, output, session) {
  ...
  # Detect saved workout types
  WorkoutTypes <- shiny::reactive({
    
    # Extract workout details in summary list
    WorkoutTypesTable <- fWorkoutTypesTable(UserID(), DBCon, Settings)
    
  })
  
  ## Summary workout types
  SummaryWorkoutTypes <- shiny::reactive({
    
    WorkoutTypeNames <- WorkoutTypes() %>%
      
      dplyr::distinct(WorkoutType) %>%
      
      dplyr::pull()
      
    purrr::map(WorkoutTypeNames, 
               function(pWorkoutType, DF) {
                 
                 WorkoutDF <- DF %>%
                   
                   dplyr::filter(WorkoutType == pWorkoutType) %>%
                   
                   # As activities may repeat, identify exercise by row order
                   dplyr::mutate(Grouping = (Activity != lag(Activity)) %>% 
                                   dplyr::if_else(is.na(.), TRUE, .), 
                                 ExerciseGroup = cumsum(Grouping))
                 
                 WorkoutDF %>%
                   
                   dplyr::group_by(ExerciseGroup, Activity) %>%
                   
                   dplyr::summarise(MaxSet = max(Set, na.rm = TRUE),
                                    .groups = "drop") %>%
                   
                   dplyr::arrange(ExerciseGroup) %>%
                   
                   dplyr::mutate(FinalOutput = paste(MaxSet, "x", Activity)) %>%
                   
                   dplyr::pull(FinalOutput) %>%
                   
                   {shiny::tagList(
                     purrr::map2(., seq_len(length(.)),
                                 function(ShortDescript, Position, FinalTag) {
                                   
                                   shiny::tagList(shiny::span(ShortDescript),
                                                  if(Position == FinalTag) NULL else shiny::br())
      ...
      purrr::set_names(nm = WorkoutTypeNames)
  ...
  output$WorkoutPageUI <- shiny::renderUI({
    ...
    if (ActiveWorkout$Status()) {
      ...
      ActiveWorkoutSection <- shiny::tagList(
        ...
        shiny::tags$u(shinyMobile::f7Padding(shiny::h2("Active workouts", 
                                                       style = paste0("color: ", Settings$ColourTheme), 
                                                       .noWS = "after"), side = "left")),
        # TODO: Add purrr for number of workouts active
        shinyMobile::f7Card()
        # Add same text for 'or choose another workout to start'
      )
    ...
    WorkoutTypeData <- SummaryWorkoutTypes()
    if (!purrr::is_empty(WorkoutTypeData)) {
      
      
      ExistingWorkoutListSection <- shiny::tagList(
        purrr::imap(WorkoutTypeData,
                    function(WorkoutData, WorkoutName) {
                      
                      shinyMobile::f7Card(title = WorkoutName,
                                          shiny::span(WorkoutData),
  ...
  shiny::observeEvent(input$CreateNewWorkoutSaveBtn, {
    ...
    # TODO: validate other inputs - name entered for exercise, numeric values in matrix
